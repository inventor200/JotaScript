        ...
        for (let i = 0; i < argArray.length; i++) {
            ...
            if (!isStaticArg(argArray[i], ignoredSubstitutions)) {
                if (specializedDeterminer) {
                    ...
                    const suggestedTransformer = specializedDeterminer(argArray);
...
class JotaBridge {
    ...
    a_switch(...args) {
        return new Program('@switch', (context, args) => {
            ...
            if (isStaticArg(lastArg) || (typeof lastArg) === 'string') {
                const lastOutcome = reduceData(testContext, lastArg);
                for (let i = 2; i < args.length - 1; i += 2) {
                    const arg = args[i];
                    if (!(isStaticArg(arg) || (typeof arg) === 'string')) {
                        return undefined;
                    }
    ...
    a_strcheck(...args) {
        return new Program('@strcheck', (context, args) => {
            ...
            if (isStaticArg(args[contentIndex])) {
    ...
    a_strloop(...args) {
        return new Program('@strloop', (context, args) => {
            ...
            return res;
        }, args).deepenContext().evaluationStatic((args) => {
            if (isStaticArg(args[0])) {
                const testContext = this.createTestContext();
    ...
    a_let(...args) { //TODO: Make a specialized determiner like a_countloop
