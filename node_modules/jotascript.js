...
function isStaticArg(arg) {
    ...
    if (dataType === 'string') {
        const varyingExp = /(?:%[a-z0-9%](?:$|\s)|%{[a-z0-9]+})/gi;
        return !converted.match(varyingExp);
    }
    if (arg.isProgram) return arg.isStatic;
    return false;
}
...
function compileContent(context, content) {
...
class Program {
    ...
    compile(context) {
        if (this.isStatic) {
            return compileContent(context, this.execute(context));
        }
        let res = this.name + '(';
        const argArray = this.simplifyArgArray(this.args);
        for (let i = 0; i < argArray.length; i++) {
            if (i > 0) res += ',';
            res += compileContent(context, argArray[i]);
        }
        return res + ')';
    }
...
class DatabaseObject {
    ...
    outputField(context, fieldName) {
        ...
        for (let i = 0; i < this.fields.length; i++) {
            const field = this.fields[i];
            if (field.fieldName === fieldName) {
                context.agent.processMessage(field.getOutput(context));
                return;
            }
        }
    }
...
class ObjectField {
    ...
    compile(context) {
        ...
        if (this.value.isSequence) {
            for (let i = 0; i < this.value.programs.length; i++) {
                const prog = this.value.programs[i];
                if (i > 0) {
                    const lastProg = this.value.programs[i-1];
                    if (!lastProg.isStatic || !prog.isStatic) {
                        res += ';';
                    }
                }
                res += this.compileSingle(context, prog);
            }
        }
        else {
    ...
    compileSingle(context, jotacode) {
        if (jotacode.isProgram) {
            return String(jotacode.compile(context));
        }
        return String(jotacode);
    }
}
...
class JotaBridge {
    ...
    a_fieldloop(...args) {
        return new Program('@fieldloop', (context, args) => {
            ...
            return res;
        }, args).deepenContext().evaluationStatic();
    }
