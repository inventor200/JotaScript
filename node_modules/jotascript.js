...
function reduceData(context, obj, doSubstitutions=true) {
    ...
    if ((typeof converted) === 'string' && doSubstitutions) {
...
class JotaBridge {
            ...
            do {
                ...
                for (let i = 0; i < this.registeredSubstitutions.length; i++) {
                    ...
                    if (pair.value && pair.value.isProgram) {
                        ...
                        if (sample === pair.key) {
                            return reduceData(context, pair.value, false);
                        }
    ...
    doFullReplacement(str, goal, replacement, ignoreCase) {
    ...
    output(programs, context=undefined, dumpMessagesAfter=true) {
        ...
        const res = this.outputItem._executeField(context, 'run');
    ...
    a_fieldloop(...args) {
        return new Program('@fieldloop', (context, args) => {
            ...
            const fieldPrefix = String(
                reduceData(context, args[1], false)
            );
            ...
            for (let i = 0; i < fieldParent.fields.length; i++) {
                ...
                res += String(reduceData(context, iterProgram.execute(context)));
            }
    ...
    a_strloop(...args) {
        return new Program('@strloop', (context, args) => {
            ...
            const substKey = String(reduceData(context, args[1], false));
            const iterProgram = args[2];
    ...
    a_countloop(...args) {
        return new Program('@countloop', (context, args) => {
            ...
            for (let i = start; i < limit; i++) {
                ...
                res += String(reduceData(context, iterProgram));
            }
    ...
    a_substitute(...args) {
        return new Program('@substitute', (context, args) => {
            ...
            return String(reduceData(context, args[0]));
        }, args);
    }
    ...
    a_replace(...args) {
        return new Program('@replace', (context, args) => {
            ...
            const str = String(reduceData(context, args[0], false));
            const goal = String(reduceData(context, args[1], false));
            const replacement = String(reduceData(context, args[2], false));

            if (str.length === 0) return str;
            if (goal.length === 0) return str;

            return reduceData(this.doFullReplacement(
    ...
    a_eq(...args) {
        return new Program('@eq', (context, args) => {
            ...
            const first = String(reduceData(context, args[0]));

            for (let i = 1; i < args.length; i++) {
                const comp = String(reduceData(context, args[i]));
    ...
    a_lt(...args) {
        return new Program('@lt', (context, args) => {
            ...
            const first = forceNumber(reduceData(context, args[0]));
            const second = forceNumber(reduceData(context, args[1]));
    ...
    a_and(...args) {
        return new Program('@and', (context, args) => {
            ...
            for (let i = 0; i < args.length; i++) {
                const comp = forceBool(reduceData(context, args[i]));
                if (comp === 0) return 0;
            }
    ...
    a_not(...args) {
        return new Program('@not', (context, args) => {
            ...
            return forceBool(reduceData(context, args[0])) === 0 ? 1 : 0;
        }, args);
    }
    ...
    a_xor(...args) {
        return new Program('@xor', (context, args) => {
            ...
            const first = forceBool(reduceData(context, args[0]));
            const second = forceBool(reduceData(context, args[1]));
    ...
    a_add(...args) {
        return new Program('@add', (context, args) => {
            ...
            for (let i = 0; i < args.length; i++) {
                res += forceNumber(reduceData(context, args[i]));
            }
    ...
    a_rand(...args) {
        return new Program('@rand', (context, args) => {
            ...
            const max = forceNumber(reduceData(context, args[0]));
    ...
    a_let(...args) {
        return new Program('@let', (context, args) => {
            ...
            //const proto = reduceData(context, subProgram, true);

            const pairs = [];
            for (let i = 0; i < args.length - 1; i += 2) {
                const pair = {
                    key: '%' + String(reduceData(context, args[i], false)),
                    //value: String(reduceData(context, args[i+1], false))
                    value: args[i+1]
                };
                pairs.push(pair);
